        ...
        <div className="bg-gray-100 min-h-screen flex flex-col items-center justify-center py-10">
          <div className="container max-w-4xl bg-white p-6 rounded-lg shadow-lg">
            ...
            {ticketDetails.length > 0 ? (
                ...
                {ticketDetails.map((ticket) => (
                  <div key={ticket._id} className="border p-4 rounded-lg">
                    <div className="flex flex-col md:flex-row justify-between items-center">
                      <div>
                        <p className="font-semibold">{ticket.ticket.passengerName}</p>
                ...
                <p className="text-center text-gray-500">Looks like you have no tickets.</p>
              )}
            </>) : (
                <div className="mt-6">
                <h2 className="text-2xl font-semibold text-center mb-4">Booking History</h2>
                {bookings.length > 0 ? (
                  <ul  className="space-y-4">
                    {bookings.map((booking) => (
                      <li key={booking.booking._id} className="border p-4 rounded-lg">
                        <div onClick={()=>showTicketsHandler(booking.booking._id)} className="flex flex-col md:flex-row justify-between items-center">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {booking.path.departure} to {booking.path.destination}
                            </p>
                            <p className="text-sm text-gray-500">
                              Date: {moment(booking.bookedAt).format("MMMM D, YYYY, HH:mm")}
                            </p>
                            <p className="text-sm text-gray-500">
                  ...
                  <p className="text-center text-gray-500">Looks like you have no bookings.</p>
                )}
              </div>
            )}
